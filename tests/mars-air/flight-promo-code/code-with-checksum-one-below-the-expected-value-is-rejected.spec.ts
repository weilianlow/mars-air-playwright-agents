...
test.describe('Checksum Validation (AC3)', () => {
  test('Code with checksum one below the expected value is rejected (BVA: below boundary)', async ({ page }) => {
    ...
    // 4. Enter "AF3-FJK-417" in the "Promotional Code" field (expected checksum=8, code has 7)
    await marsAirPage.promoCodeInput.fill('AF3-FJK-417');

    // 5. Click the "Search" button
    await marsAirPage.searchButton.click();

    await expect(page.getByText('Sorry, code AF3-FJK-417 is not valid')).toBeVisible();
